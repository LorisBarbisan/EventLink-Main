...
export function ProfileQRCode({ userId, profileUrl, modal = false }: Props) {
  ...
  const content = (
    <div className="flex flex-col items-center gap-4">
        ...
        {imgError ? (
          <div className="flex h-48 w-48 flex-col items-center justify-center gap-2 text-muted-foreground">
            ...
            <p className="text-center text-xs">
              QR generation requires
              <br />
              installing dependencies.
            </p>
            <p className="text-center text-xs text-primary">Run: npm install</p>
          </div>
        ) : (
          <>
            {!imgLoaded && (
      ...
      <div className="flex w-full flex-col gap-2 sm:flex-row">
        ...
        <Button className="flex-1" onClick={handleDownload} disabled={imgError || !imgLoaded}>
