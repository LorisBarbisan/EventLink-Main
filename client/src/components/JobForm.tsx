      ...
      <CardContent className="space-y-4">
        ...
        <Collapsible open={showOptional} onOpenChange={setShowOptional}>
          <CollapsibleTrigger asChild>
            <button
              type="button"
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors w-full py-2 border-t mt-2"
            >
              <ChevronDown className={`h-4 w-4 transition-transform ${showOptional ? "rotate-180" : ""}`} />
              Additional Details {hasOptionalData && <span className="text-xs bg-muted px-1.5 py-0.5 rounded">has content</span>}
            </button>
          </CollapsibleTrigger>
