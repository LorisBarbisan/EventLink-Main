...
export async function seedProductionJobs(): Promise<void> {
  try {
    ...
    if (!adminUser) {
      console.log("⚠️ Admin user not found, skipping job seed");
      return;
    }
    ...
    const jobs = [
      ...
      },
      {
        ...
        type: "published",
        rate: "£250/day",
        description: "Seeking a professional event photographer to capture candid and posed shots at a major awards ceremony. You will deliver edited high-resolution images within 48 hours. Portfolio showcasing event photography experience required.",
        status: "active",
      ...
      },
      {
        ...
        type: "published",
        rate: "£500/event",
        description: "Charismatic and professional event host needed for a product launch. You will introduce speakers, manage audience engagement, and keep the event flowing smoothly. Public speaking experience and confident stage presence essential.",
        status: "active",
    ...
    console.log(`🌱 Seeding ${jobs.length} production jobs...`);

    for (const job of jobs) {
      await storage.createJob(job as any);
    }

    console.log("✅ Production jobs seeded successfully");
  } catch (error) {
    console.error("❌ Failed to seed production jobs:", error);
  }
}
