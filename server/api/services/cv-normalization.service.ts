...
const VAGUE_SKILLS = new Set([
  "hard-working", "hardworking", "motivated", "team player", "experienced",
  "dedicated", "passionate", "reliable", "flexible", "adaptable",
  "good communicator", "communication skills", "problem solver",
  "attention to detail", "self-motivated", "proactive", "enthusiastic",
]);
...
export class CvNormalizationService {
  normalizeAll(raw: Record<string, any>): NormalizedFields {
    ...
    if (raw.fullName?.value) {
      ...
      if (v) result.fullName = { value: v, confidence: raw.fullName.confidence ?? 0.8, source: raw.fullName.source ?? "header" };
    }

    if (raw.title?.value) {
      const v = this.cleanTitle(raw.title.value);
      if (v) result.title = { value: v, confidence: raw.title.confidence ?? 0.75, source: raw.title.source ?? "header" };
    }

    if (raw.location?.value) {
      const v = this.cleanLocation(raw.location.value);
      if (v) result.location = { value: v, confidence: raw.location.confidence ?? 0.7, source: raw.location.source ?? "header" };
    }

    if (raw.bio?.value) {
      const v = this.cleanBio(raw.bio.value);
      if (v) result.bio = { value: v, confidence: raw.bio.confidence ?? 0.85, source: raw.bio.source ?? "summary" };
    }

    if (raw.experienceYears?.value) {
      const v = this.cleanExperienceYears(raw.experienceYears.value);
      if (v !== undefined) result.experienceYears = { value: v, confidence: raw.experienceYears.confidence ?? 0.7, source: raw.experienceYears.source ?? "summary" };
    }

    if (raw.skills?.value) {
      const v = this.cleanSkills(raw.skills.value);
      if (v?.length) result.skills = { value: v, confidence: raw.skills.confidence ?? 0.85, source: raw.skills.source ?? "skills" };
    }

    if (raw.certifications?.value) {
      const v = this.cleanCertifications(raw.certifications.value);
      if (v?.length) result.certifications = { value: v, confidence: raw.certifications.confidence ?? 0.8, source: raw.certifications.source ?? "certifications" };
    }

    if (raw.workHistory?.value) {
      const v = this.cleanWorkHistory(raw.workHistory.value);
      if (v?.length) result.workHistory = { value: v, confidence: raw.workHistory.confidence ?? 0.8, source: raw.workHistory.source ?? "experience" };
    }

    if (raw.education?.value) {
      const v = this.cleanEducation(raw.education.value);
      if (v?.length) result.education = { value: v, confidence: raw.education.confidence ?? 0.8, source: raw.education.source ?? "education" };
    }
  ...
  cleanFullName(val: any): string | undefined {
    ...
    if (name.includes("@") || /\d{5,}/.test(name) || /curriculum vitae/i.test(name)) return undefined;
  ...
  cleanTitle(val: any): string | undefined {
    ...
    const badPrefixes = ["in ", "for ", "the ", "a ", "an ", "this ", "with ", "and ", "to ", "of ",
      "experienced", "passionate", "dedicated", "skilled", "qualified", "highly"];
