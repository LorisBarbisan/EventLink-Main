...
- **Reference & Reputation System**: Freelancers can generate a unique token-based reference link (via "Build My Reputation" in account menu) to share with past clients. Recipients complete a 3-question form at `/reference/:token` (no login required, ~45 seconds). Q1: confirmed working together (boolean); Q2: reliability/professionalism (excellent/good/mixed/prefer not to say); Q3: would work again (absolutely/yes/unlikely/prefer not to say). Badge logic: Q1 false → flagged (private); prefer_not_to_say → work_history_confirmed; Q2 excellent + Q3 absolutely → highly_recommended; Q2 good or Q2 excellent + Q3 yes → recommended; mixed/unlikely → verified_private (not shown publicly). Badges and reference comments appear on freelancer public profiles. Email notification sent to freelancer when reference is submitted. Backend: `freelancer_references` table, `reference_token` on `freelancer_profiles`, API routes at `/api/references/*`.
