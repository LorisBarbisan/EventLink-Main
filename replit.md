...
- **Admin Jobs Management Tab**: Admin dashboard includes a dedicated "Jobs" tab with paginated table showing all jobs (title, company, location, status, source, application count, hired count, event dates, posted by, created date). Filters include search (title/company/location), status (active/paused/closed/private), type (EventLink/External), sort by (date/title/company/status), and sort order. Backend endpoint `GET /api/admin/jobs` with server-side pagination.
